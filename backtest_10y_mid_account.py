"""
Comprehensive 10-Year Autonomous Backtest on a $50,000 Mid Account (2016 - 2026).
Features:
- Initial Capital: $50,000.00 (Tier 2 Mid Account)
- Dynamic Tier Scaling (4 positions @ 24% each -> scales to 8 positions past $100k)
- 200-EMA Macro Bear Cash Shield
...
def run_10y_mid_backtest(initial_capital: float = 50000.0):
    ...
    print("\n" + "="*90)
    print(f"  RUNNING 10-YEAR AUTONOMOUS BACKTEST ON ${initial_capital:,.2f} MID ACCOUNT (2016 - 2026)")
    print("="*90)
            ...
            for symbol, df in symbol_data.items():
                ...
                if signal and signal.direction == "BUY":
                    ...
                    if shares > 0 and (shares * signal.suggested_entry) <= engine.cash:
                        order = Order(
                            order_id=f"mid10y_{int(ts.timestamp())}_{symbol}",
                            symbol=symbol,
                            direction="BUY",
                            order_type="MARKET",
                            shares=shares,
    ...
    # 1. Overall 10-Year Mid Account Summary Table
    print("\n" + "="*90)
    print("  1. $50,000 MID ACCOUNT: 10-YEAR OVERALL PERFORMANCE (2016 - 2026)")
    print("="*90)
    overall_table = [
    ...
    print(tabulate(overall_table, headers=["Metric", "$50k Mid Account Value"], tablefmt="fancy_grid"))

    # 2. Year-by-Year Compounding Table
    print("\n" + "="*90)
    print("  2. $50,000 MID ACCOUNT: YEAR-BY-YEAR COMPOUNDING MILESTONES")
    print("="*90)
    trades_by_year = {}
    ...
    # 3. Top 10 Winning Trades on $50k Account
    print("\n" + "="*90)
    print("  3. TOP 10 LARGEST REALIZED WINNING TRADES ON $50k MID ACCOUNT")
    print("="*90)
...
if __name__ == "__main__":
    run_10y_mid_backtest(initial_capital=50000.0)
